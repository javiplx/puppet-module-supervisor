...
describe 'supervisor::service', :type => :define do
  ...
  context "with redhat configuration" do
    let (:facts) { {
      :osfamily => 'redhat',
    } }

    it "should include /etc/supervisord.d/*.ini in /etc/supervisord.conf" do
      should contain_file('/etc/supervisord.conf') \
          .with_content(Regexp.new Regexp.escape 'files = /etc/supervisord.d/*.ini')
    end

    it "should create /etc/supervisord.d/sometitle.ini" do
      should create_file('/etc/supervisord.d/sometitle.ini') \
          .with_content(Regexp.new Regexp.escape 'command=somecommand')
    end
    ...
    context "with ensure => absent" do
      ...
      it "should delete config and log dir" do
        should contain_file('/etc/supervisord.d/sometitle.ini').with_ensure('absent')
